...
dataBase = mysql.connector.connect(
    host = "localhost",     # the server name or Ip address on which MySQL is running.
    ...
    passwd = "1234"   # password is given by the user at the time of installing the MySQL server.

)
cursorObject = dataBase.cursor()  # preparing a cursor object

cursorObject.execute("CREATE DATABASE IF NOT EXISTS marvel_db")  # creating database
...
def addButton():
    ...
    cancel_button = Button(popup_window, text="Cancel", command=cancelButton)
...
add_button = Button(text="Add", command=addButton, height=2, width=15)
...
# create the text box
text_box = Text(master, height=30, width=83)
text_box.place(x=5, y=60)

# create the "LIST ALL" button
listall_button = Button(master, text="LIST ALL", command=listallButton, height=2, width=15)
